...
docker
"
for cmd in $COMMAND_DEPENDENCIES; do
  command -v "$cmd" > /dev/null || (echo "ERROR $0: Requires the '$cmd' command to be installed" && exit 1)
done

local_s3_dir="$(realpath "$(dirname "$0")")"
...
local_chillbox_app_key_id="local-chillbox-app-key-id"
local_chillbox_secret_access_key="local-secret-access-key-with-readwrite-policy"
...
while true; do
  ...
  if [ "${chillbox_minio_state}" = "true" ]; then
    ...
    echo "Success: mc admin info local"
...
DOCKER_BUILDKIT=1 docker build -t chillbox-local-shared-secrets - < "${local_s3_dir}/local-shared-secrets.Dockerfile"
docker run -d --rm \
...
while true; do
  chillbox_local_shared_secrets_state="$(docker inspect --format '{{.State.Running}}' chillbox-local-shared-secrets)"
  if [ "${chillbox_local_shared_secrets_state}" = "true" ]; then
    echo "done"
    break
  else
    printf "."
...
docker exec --user root chillbox-local-shared-secrets mkdir -p /var/lib/chillbox-shared-secrets/chillbox-minio
docker exec --user root chillbox-local-shared-secrets chmod -R 700 /var/lib/chillbox-shared-secrets/chillbox-minio
docker exec --user root chillbox-local-shared-secrets chown -R dev:dev /var/lib/chillbox-shared-secrets/chillbox-minio
